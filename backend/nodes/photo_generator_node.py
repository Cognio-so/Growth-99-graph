...
async def photo_generator(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    image_requirements = await _analyze_image_requirements_with_llm(
        user_text, json_schema, state
    )
...
async def _analyze_image_requirements_with_llm(
    user_text: str, json_schema: dict, state: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
...
"""
    ...
    try:
        ...
        if json_match:
            ...
            image_requirements = [
                req for req in image_requirements if req.get("category") != "logo"
            ]
...
def _fallback_detailed_analysis(
    user_text: str, json_schema: dict, state: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    ...
    if any(
        ...
        for word in ["hospital", "medical", "health", "clinic", "dental"]
    ):
        website_type = "healthcare/hospital website"
    elif any(
        word in user_text_lower for word in ["restaurant", "food", "menu", "dining"]
    ):
        website_type = "restaurant/food website"
    elif any(
        word in user_text_lower
        for word in ["shop", "store", "product", "ecommerce", "buy"]
    ):
        ...
        word in user_text_lower
        for word in ["real estate", "property", "house", "apartment"]
    ):
...
async def _intelligent_check_existing_images(
    image_requirements: List[Dict[str, Any]], state: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    ...
    needed_images = await _batch_check_existing_images(
        image_requirements, existing_images, state
    )

    return needed_images


async def _batch_check_existing_images(
        ...
        existing_data.append(
            {
                ...
                "url_count": len(
                    [url for url in [img.get("url1"), img.get("url2")] if url]
                ),
            }
        )
        ...
        if json_match:
            ...
            for req in image_requirements:
                ...
                if not has_suitable:
                    ...
                    print(
                        f"✅ Found suitable existing images for: {description} ({req['category']})"
                    )
...
def _generate_and_save_new_images(needed_images: List[Dict[str, Any]]):
    ...
    for req in needed_images:
        ...
        image_urls = _generate_high_quality_images_from_pexels(
...
def _generate_high_quality_images_from_pexels(
    description: str, context: str, category: str, max_images: int = 1
) -> List[str]:
    """
    ...
    for search_query in search_strategies:
        try:
            params = {
                ...
                "per_page": max_images,
                "orientation": (
                    "landscape" if category in ["banner", "photo"] else "all"
                ),
            }

            response = requests.get(
                f"{PEXELS_BASE_URL}/search", headers=headers, params=params
            )
...
def _infer_category_from_description(description: str) -> str:
    ...
    if any(
        word in description_lower for word in ["logo", "brand", "monogram", "wordmark"]
    ):
        return "logo"
    elif any(
        word in description_lower for word in ["banner", "hero", "background", "scenic"]
    ):
        return "banner"
    elif any(
        word in description_lower for word in ["icon", "badge", "symbol", "line icon"]
    ):
        return "icon"
    else:
        return "photo"
...
def _generate_placeholder_urls(
    description: str, category: str, count: int
) -> List[str]:
    """
...
def _save_image_to_csv(
    description: str, website_type: str, context: str, category: str, urls: List[str]
):
    """
    ...
    if not IMAGES_CSV_PATH.exists():
        with open(IMAGES_CSV_PATH, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["s_no", "description", "website_type", "context", "url1", "url2"]
            )
...
async def _load_relevant_images_from_csv(
    image_requirements: List[Dict[str, Any]], state: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    ...
    matching_results = await _batch_find_matching_images(
        image_requirements, all_images, state
    )
...
async def _batch_find_matching_images(
    image_requirements: List[Dict[str, Any]],
    ...
    try:
        ...
        if json_match:
            ...
            for req in image_requirements:
                ...
                matching_images = [
                    img for img in all_images if int(img["s_no"]) in matching_s_nos
                ]
...
def _add_uploaded_logo_to_images(
    gi: Dict[str, Any], final_images: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
...
def _add_uploaded_image_to_images(
    gi: Dict[str, Any], final_images: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
