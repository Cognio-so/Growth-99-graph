...
async def _extract_vision_design_specs(
    image_url: str, user_query: str
) -> Dict[str, Any]:
    """
...
"""
            llm_result = await call_llm_json(
                chat,
                "You are an intent classifier for image usage.",
                classification_prompt,
            )

            if isinstance(llm_result, dict) and llm_result.get("intent") in [
                "direct_usage",
                "vision_analysis",
            ]:
            ...
            print(
                f"⚠️ LLM intent classification failed, falling back to keyword match: {e}"
            )
        ...
        vision_keywords = [
            ...
            "like this",
            "make it like this",
            "make the hero section like this",
            "match this",
            "match this style",
            "copy this design",
            "recreate this",
    ...
    try:
        ...
        try:
            ls_result = sandbox.commands.run(
                "find my-app/src/components -name '*.jsx' -o -name '*.js'", timeout=10
            )
            if ls_result.stdout:
                    ...
                    if file_path and "my-app/" in file_path:
                        ...
                        try:
                            ...
                            if content:
                                ...
                                print(
                                    f"   ⚠️ Empty content for component: {relative_path}"
                                )
                        except Exception as e:
                            print(
                                f"   ❌ Could not capture component {relative_path}: {e}"
                            )
            else:
...
async def _detect_image_requirements(
    user_text: str, existing_code: str
) -> List[Dict[str, Any]]:
    """
...
"""
        ...
        result = await call_llm_json(
            model,
            "You are an image requirements analyzer. Output only valid JSON.",
            prompt,
        )
...
async def _generate_images_for_edit(
    image_requirements: List[Dict[str, Any]], state: GraphState
) -> List[Dict[str, Any]]:
    """
...
async def edit_analyzer(state: GraphState) -> GraphState:
    ...
    if not user_text:
        ...
        ctx["edit_analysis"] = {"error": "No user text provided for edit analysis"}
        state["context"] = ctx
        return state
    ...
    try:
        ...
        if has_document_info:
            ...
            user_prompt += (
                f"- Business Name: {extraction.get('business_name', 'Not specified')}\n"
            )
            user_prompt += (
                f"- Brand Name: {extraction.get('brand_name', 'Not specified')}\n"
            )
            user_prompt += f"- Value Proposition: {extraction.get('unique_value_proposition', 'Not specified')}\n"
            user_prompt += (
                f"- Color Palette: {extraction.get('color_palette', 'Not specified')}\n"
            )
            user_prompt += f"- Font Style: {extraction.get('preferred_font_style', 'Not specified')}\n"
            user_prompt += (
                f"- Logo URL: {extraction.get('logo_url', 'Not specified')}\n"
            )
        ...
        edit_analysis = {
            ...
            "content_preservation_rules": result.get(
                "content_preservation_rules",
                [
                    "Preserve all text content and functionality",
                    "Protect images from theme color overlays",
        ...
        if image_requirements and not image:
            ...
            generated_images = await _generate_images_for_edit(
                image_requirements, state
            )
            ...
            edit_analysis["image_requirements"] = image_requirements
            edit_analysis["generated_images"] = []
        ...
        if image:
            ...
            if image_url:
                ...
                if intent_analysis["use_for_vision"]:
                    ...
                    chat = await get_chat_model("groq-default", temperature=0.1)
                    edit_text = (
                        await chat.ainvoke([{"role": "user", "content": edit_prompt}])
                    ).content
                    ...
                    image_requirements_from_vision = _detect_image_requirements(
                        edit_text, existing_code
                    )

                    generated_images_from_vision = []
                    if image_requirements_from_vision:

                        generated_images_from_vision = await _generate_images_for_edit(
                            image_requirements_from_vision, state
                        )
            ...
            gi.update(
                {
                    ...
                    "extracted_unique_value_proposition": extraction.get(
