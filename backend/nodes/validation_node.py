...
def validate_code(state: Dict[str, Any]) -> Dict[str, Any]:
    """Validate generated code with AUTOMATIC ERROR RECOVERY."""
    print("--- Running Validation Node ---")
    ...
    try:
        ...
        if _global_sandbox:
            ...
            print("⚠️ No sandbox available for validation")
            validation_errors.append({
                "type": "no_sandbox",
                "message": "No sandbox available for validation",
                "severity": "critical"
            })
    ...
    if not validation_errors:
        ...
        # Capture files for correction
        correction_data = _capture_generated_files_for_correction(ctx)  # FIX: Use correct function name
